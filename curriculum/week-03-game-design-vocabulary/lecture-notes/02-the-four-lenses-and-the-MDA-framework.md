...
mindmap
  root(("Four Lenses"))
    ...
    Goals
```
*Church's four lenses are independent angles for evaluating a design, not a sequence to follow in order.*
...
```mermaid
flowchart LR
  subgraph Designer["Designer reads"]
    M1["Mechanics"] --> D1["Dynamics"] --> A1["Aesthetics"]
  end
  subgraph Player["Player reads"]
    A2["Aesthetics"] --> D2["Dynamics"] --> M2["Mechanics"]
  end
```
*The designer builds top-down from mechanics; the player experiences it bottom-up starting from aesthetics.*
