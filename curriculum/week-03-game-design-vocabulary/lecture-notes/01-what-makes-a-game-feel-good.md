...
```mermaid
flowchart TD
  A["Real-time control"] --> D["Game feel"]
  B["Simulated space with rules"] --> D
  C["Polish"] --> D
  D --> E["Feels good"]
```
*Game feel is built from three pillars; feeling good is the outcome, not one of the inputs.*
...
```mermaid
sequenceDiagram
  participant Handler as Collision handler
  participant Bus as Event bus
  participant Shake as Screen shake
  participant Particles as Particles
  participant Audio as Audio

  Handler->>Bus: emit brick destroyed
  Bus->>Shake: trigger
  Bus->>Particles: spawn burst
  Bus->>Audio: play
```
*One emitted event fans out to three independent listeners instead of three separate direct calls.*
