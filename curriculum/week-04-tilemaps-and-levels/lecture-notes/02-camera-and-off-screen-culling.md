...
```mermaid
flowchart TD
  A["Read player position"] --> B["Follow camera snap lerp or dead zone"]
  B --> C["Clamp camera to level bounds"]
  C --> D["Compute visible tile range cull"]
  D --> E["Draw culled tiles"]
```
*The per-frame camera order: follow, then clamp, then cull, then draw.*
...
```mermaid
flowchart LR
  A["Background layer half camera offset"] --> B["Foreground layer full camera offset"]
  B --> C["Entities"]
  C --> D["HUD screen space no camera"]
```
*Painter's-algorithm draw order: back to front, HUD last and unshaken.*
