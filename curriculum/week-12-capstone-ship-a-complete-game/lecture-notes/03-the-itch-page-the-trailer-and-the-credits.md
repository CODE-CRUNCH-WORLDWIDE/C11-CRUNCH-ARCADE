...
itch.io itself recommends Free for first ships in its *Getting Started* docs and in the community forums; the recommendation is empirically grounded in data from the platform's first decade of operation. The learners who shipped for free in past cohorts have a public artefact; the learners who priced their first build at $5 have, on average, fewer downloads, no revenue, and a less linkable portfolio.
...
```mermaid
flowchart LR
  A["Thumbnail"] --> B{"Visitor clicks in"}
  B -- Yes --> C["Trailer and screenshots"]
  C --> D{"Visitor downloads"}
  B -- No --> E["Visitor moves on"]
  D -- No --> E
  D -- Yes --> F["Game downloaded"]
```
*The thumbnail earns the click; the trailer earns the download.*
...
```mermaid
flowchart TD
  A["CREDITS md source of truth"] --> B["In game credits screen"]
  A --> C["itch io page credits section"]
```
*CREDITS.md is the single source mirrored into the game and the page.*
