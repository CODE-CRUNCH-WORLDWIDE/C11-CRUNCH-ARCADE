...
```mermaid
flowchart LR
  A["Godot Editor"] --> B["Export templates"]
  B --> C["export presets cfg"]
  C --> D["HTML5 build"]
  C --> E["Windows build"]
  C --> F["macOS build"]
  C --> G["Linux build"]
```
*The export pipeline from editor to four shipped binaries.*
...
- **OpenProcessing** ([openprocessing.org](https://openprocessing.org)). p5.js / Processing only, so not a Godot fit; mentioned for completeness because some capstone learners come from a creative-coding background and want to know.
...
That is a 35-minute Thursday export pass. The learners who installed the export templates on Monday do this in 35 minutes. The learners who install them on Thursday at 10:00 sharp do it in two hours after the download finishes.
