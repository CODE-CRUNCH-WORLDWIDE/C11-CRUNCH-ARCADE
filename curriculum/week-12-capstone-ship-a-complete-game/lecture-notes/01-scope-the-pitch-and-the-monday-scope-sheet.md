...
The single most consequential decision of the capstone week is made on Monday morning, before any code is written. It is not a technical decision. It is a *scope* decision. The learners who finish this week are not the ones with the most ambitious concept; they are the ones whose Monday concept was small enough that the Sunday build is recognisably the same project. This lecture is about how to make that decision well, how to write it down so you do not drift, and how to defend the decision against the predictable Wednesday-afternoon temptation to add one more thing.
...
```mermaid
flowchart TD
  A["Tuesday end of day"] --> B{"Verb works"}
  B -- No --> F["Rescope drop a feature"]
  B -- Yes --> C{"Goal works"}
  C -- No --> F
  C -- Yes --> D{"Obstacle works"}
  D -- No --> F
  D -- Yes --> E["Gate cleared move to art and juice"]
  F --> B
```
*The Tuesday playable prototype gate as a pass or rescope decision.*
...
Cutting is a learnt skill. The mistake most learners make on a first capstone week is treating every feature in the Monday brainstorm as a commitment. It is not. The Monday brainstorm is a *menu*. The Monday scope sheet is what you ordered. The cuts log is what you sent back.
