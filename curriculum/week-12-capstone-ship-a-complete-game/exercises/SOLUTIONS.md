...
The point of the exercise is to give you a *mechanical* check against the ten fields prescribed in Lecture 1. The capstone graders will accept scope sheets in any format, but the empirical data is that learners whose Monday scope sheet passes this validator are 70%+ likely to ship by Sunday, and learners whose sheet fails it are 40% likely to be in a Wednesday-afternoon emergency rescope.
...
Common learner errors on this exercise:
...
- The "missing target" check uses the Godot platform names exactly as the editor writes them (`Web`, `Windows Desktop`, `macOS`, `Linux/X11`). Learners sometimes hand-edit the file and use shorter names; the validator catches the mismatch.
