...
classDiagram
  ...
  State <|-- IdleState
  State <|-- RunState
  AirborneState <|-- JumpState
  AirborneState <|-- FallState
  AirborneState <|-- WallSlideState
```
*JumpState, FallState, and WallSlideState inherit shared airborne physics from one AirborneState parent instead of duplicating it.*
...
```mermaid
flowchart TD
  A["Stack top is Idle"] -->|push PauseState on ESC| B["Stack top is Pause and Idle suspended beneath"]
  B -->|pop on resume| A
  A -->|change to RunState| C["Stack top is Run, Idle gone"]
```
*Push suspends the current state under a new one; pop resumes it; change replaces it outright.*
