...
stateDiagram-v2
  [*] --> IDLE
  IDLE --> RUN : MOVE PRESSED
  RUN --> IDLE : MOVE RELEASED
  IDLE --> JUMP : JUMP PRESSED
  RUN --> JUMP : JUMP PRESSED
  IDLE --> FALL : FELL OFF EDGE
  RUN --> FALL : FELL OFF EDGE
  JUMP --> FALL : FELL OFF EDGE
  JUMP --> IDLE : LANDED
  FALL --> IDLE : LANDED
```
*The nine-transition table from Section 3 as a state diagram — note there is no JUMP to JUMP arrow, which is what makes double-jumping structurally impossible.*
...
```mermaid
flowchart TD
  A["Phase 1 - Gather events"] --> B["Phase 2 - Decide transitions"]
  B --> C["Phase 3 - Run state behaviour"]
  C --> D["Phase 4 - Render"]
  D --> A
```
*The four phases run in this fixed order, every frame, forming one update loop.*
