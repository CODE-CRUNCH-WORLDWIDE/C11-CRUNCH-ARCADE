...
sequenceDiagram
  ...
  participant ClientA
  participant ClientB
  Server->>Server: Peer connects triggers callback
  Server->>Server: Instantiate player scene and add child
  Server->>ClientA: Spawner broadcasts spawn message
  Server->>ClientB: Spawner broadcasts spawn message
  ClientA->>ClientA: Instantiate same scene locally
  ClientB->>ClientB: Instantiate same scene locally
```
*How MultiplayerSpawner turns one add child call on the server into synchronized spawns on every client.*
