...
```mermaid
flowchart TD
  A["Game traffic to send"] --> B{"Does a delay break the experience"}
  B -->|Yes| C["Use UDP"]
  B -->|No| D["Use TCP"]
  C --> E["State and events needing speed"]
  D --> F["Lobby chat matchmaking patcher saves"]
```
*The dividing question that decides UDP or TCP for any piece of game traffic.*
