...
sequenceDiagram
  participant Client
  participant Server
  Client->>Client: Predict move on input W
  Client->>Server: Send input W
  Server->>Server: Apply input to simulation
  Server-->>Client: Snapshot with authoritative position
  Client->>Client: Rewind to server position
  Client->>Client: Replay unacknowledged inputs
  Client->>Client: Render corrected position
```
*How client prediction and server reconciliation keep the local player responsive and correct.*
