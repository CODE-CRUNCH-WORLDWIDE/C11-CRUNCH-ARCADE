...
```mermaid
flowchart TD
  A["Ball moves this frame"] --> B{"Did the path cross a thin wall"}
  B -->|"No"| C["Overlap test catches it next frame"]
  B -->|"Yes tunneling"| D["Pick a fix"]
  D --> E["Clamp dt"]
  D --> F["Cap velocity"]
  D --> G["Sub step the frame"]
  D --> H["Swept AABB test Week 4"]
```
*Four ways to stop a fast body from skipping past a thin wall between frames.*
...
```mermaid
flowchart TD
  A["Collision detected"] --> B["Compute overlap x and overlap y"]
  B --> C{"overlap x smaller than overlap y"}
  C -->|"Yes"| D["Push out along x axis"]
  C -->|"No"| E["Push out along y axis"]
  D --> F["Snap position then flip vel x"]
  E --> G["Snap position then flip vel y"]
```
*Which axis to push out on is decided by whichever overlap is smaller.*
