...
```mermaid
flowchart TD
  A["Accumulate acceleration"] --> B["vel += acc times dt"]
  B --> C["pos += vel times dt"]
  C --> D{"Ball hits floor"}
  D -->|"Yes"| E["Snap position to floor"]
  E --> F["vel y = negative vel y times restitution"]
  F --> G{"abs vel y below rest threshold"}
  G -->|"Yes"| H["Snap vel y to zero"]
  G -->|"No"| I["Render frame"]
  H --> I
  D -->|"No"| I
  I --> A
```
*One frame of the integrate, bounce, and settle cycle that drives the whole simulation.*
