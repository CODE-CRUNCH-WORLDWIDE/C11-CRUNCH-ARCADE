...
**B** — Two AABBs overlap if and only if their intervals overlap on BOTH X and Y. "Or" would report overlap whenever they're in the same row or column, regardless of distance. (Lecture 1 §2.)
...
**B** — Pygame Rects are half-open: a rect with `x=10, width=20` covers `10..29` inclusive, pixel `30` is not inside. The strict-`<` overlap test matches this. (Lecture 1 §2, §3.)
...
**A** — At 2000 px/s and `dt = 1/30 s`, max movement per frame is `2000/30 ≈ 66.7 px`. The ball can entirely cross a 30-px wall in one frame and never sample a position inside it. (Lecture 1 §5.)
...
**B** — Compare `distance²` to `(radius_sum)²`. No `sqrt`. Answer C is wrong because `(a+b)² ≠ a² + b²`. (Lecture 1 §4.)
...
**B** — Forward Euler. Swap to semi-implicit (velocity-first) for strictly-better energy behaviour with gravity. (Lecture 2 §2.)
...
**B** — The rest-threshold pattern: below some small `|vel.y|` post-bounce, snap to zero. Four-line fix. (Lecture 2 §5.)
...
**A** — Smaller overlap = the axis you came in along. Push out that way and reflect that axis's velocity. Larger overlap = the axis you were already inside on, which would push too far. (Lecture 1 §6.)
...
**C** — Landing is not a bounce. Restitution = 0. Snap-and-zero. (Lecture 2 §4 and Exercise 3.)
...
**B** — The "paddle English" trick: where on the paddle the ball hit determines its outgoing horizontal velocity. Without this, Breakout would not be playable. (Challenge 1.)
...
**B** — Euler is `O(dt²)` accurate per step. At `dt ≈ 16 ms` and game-scale velocities those errors are dominated by player perception and frame quantisation. Pick the simplest integrator that doesn't visibly misbehave. (Lecture 2 §1.)
