...
**C** — Persistent state is what the player has earned; the others are session (velocity), presentation (`pygame.Surface`), and session (screen-shake timer). See Lecture 1 §2.
...
**B** — All three. The code-execution-on-load point alone disqualifies pickle for any cross-machine context; the refactoring brittleness disqualifies it across game versions; the Python-only constraint disqualifies it for any out-of-process tool. Lecture 1 §7.
...
**C** — The first byte of every save is the version stamp; without it, evolving the schema in v2 cannot read v1 saves without breaking them. Lecture 3 §1.
...
**B** — JSON is the default. MessagePack is the answer when JSON shows up on a profiler. Lecture 2 §10.
...
**B** — JSON has high lexical redundancy (key names repeated per record, numbers as ASCII digits) and gzip exploits exactly this kind of pattern. Lecture 2 §7.
...
**B** — Write, flush+fsync, rename. The fsync ensures the temp file is durable before the rename commits it; the rename is atomic. Lecture 3 §4.
...
**A** — One step at a time, update the version, return. The composer chains them. Lecture 3 §2.
...
**B** — Integrity against accident, not security against malice. SHA-256 is a checksum, not a signature; a tampered file with a recomputed hash will pass the check. Lecture 3 §6.
...
**C** — The player controls the machine; any obfuscation can be reversed. Local-save anti-cheat is theatre. The exception is multiplayer leaderboards (server-side authority), which is a Week-12 topic. Lecture 3 §7.
...
**C** — Level boundary is the safe cadence; the autosave slot is dedicated and cannot trample manual saves. Lecture 3 §8.
