...
```mermaid
flowchart LR
  A["v1 save on disk"] --> B["migrate_v1_to_v2"]
  B --> C["v2 shape - coins added"]
  C --> D["migrate_v2_to_v3"]
  D --> E["v3 shape - equipment split out"]
  E --> F["Current schema reached - stop"]
```

*The migration ladder: each function steps one version forward, and the composer chains them until the loaded save matches the current schema.*
...
```mermaid
flowchart TD
  A["Load requested"] --> B["Try save.json"]
  B --> C{"Read succeeds"}
  C -->|"Yes"| D["Return save data"]
  C -->|"No"| E["Try save.json.bak"]
  E --> F{"Read succeeds"}
  F -->|"Yes"| D
  F -->|"No"| G["Report failure - offer new game"]
```

*The `.bak` fallback chain: the primary save is tried first, the backup second, and only then does the player see a failure.*
