...
```mermaid
flowchart TD
  A["Choose a save format"] --> B{"Is this a game save file"}
  B -->|"No - network packet or replay"| C["Use MessagePack or custom binary"]
  B -->|"Yes"| D{"Save size"}
  D -->|"Under 50 KB"| E["Use JSON"]
  D -->|"Over 50 KB"| F["Use JSON plus gzip"]
  E --> G{"Hot path and profiler shows cost"}
  G -->|"Yes"| H["Switch to MessagePack"]
  G -->|"No"| I["Stay with JSON"]
```

*The three-line decision rule as a flowchart: JSON is the default until a measured cost says otherwise.*
