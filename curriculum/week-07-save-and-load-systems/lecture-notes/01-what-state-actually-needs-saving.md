...
```mermaid
flowchart TD
  A["Field found in game object"] --> B{"Survives quit and reload"}
  B -->|"No"| C{"Render or audio binding"}
  C -->|"Yes"| D["Presentation - do not save"]
  C -->|"No"| E{"Derivable from other state"}
  E -->|"Yes"| F["Derived - do not save"]
  E -->|"No"| G["Session - do not save"]
  B -->|"Yes"| H["Persistent - save it"]
```

*The same decision tree from the diagram above, as a flowchart: only the persistent branch reaches disk.*
...
```mermaid
flowchart LR
  A["Live objects Player World App"] --> B["capture_state"]
  B --> C["GameState"]
  C --> D["gamestate_to_dict"]
  D --> E["Save file on disk"]
  E --> F["gamestate_from_dict"]
  F --> G["GameState"]
  G --> H["apply_state"]
  H --> I["Live objects updated"]
```

*The full write and read boundary: live objects never touch the file format directly.*
