...
```mermaid
flowchart LR
  A["dt each frame"] --> B["elapsed plus equals dt"]
  B --> C["t equals elapsed over duration"]
  C --> D["eased equals ease of t"]
  D --> E["value equals lerp start end eased"]
```
*How Tween turns elapsed time into an eased output value each frame.*
...
```mermaid
flowchart TD
  GF["Game Feel"] --> RT["Real time control"]
  GF --> SP["Simulated physical space"]
  GF --> PO["Polish the juice pillar"]
  RT --> FOUND["Built in Weeks 1 to 5"]
  SP --> FOUND
  PO --> THISWEEK["Built in Week 6"]
```
*Swink's three pillars of game feel; polish rests on the control and physics foundation from earlier weeks.*
