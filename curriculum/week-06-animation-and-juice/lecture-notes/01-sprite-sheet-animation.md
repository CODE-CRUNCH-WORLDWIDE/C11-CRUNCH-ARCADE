...
stateDiagram-v2
  [*] --> IdleState
  IdleState --> RunState : move input
  RunState --> JumpState : jump pressed
  JumpState --> FallState : apex reached
  FallState --> IdleState : landed
  IdleState --> HurtState : took damage
  HurtState --> IdleState : invincibility ends

  note right of IdleState : enter plays idle_breathing
  note right of RunState : enter plays run_loop and footsteps loop
  note right of JumpState : enter plays jump_launch
  note right of FallState : enter plays fall_airborne
  note right of HurtState : enter plays hurt_react
```
*Each FSM state's enter hook plays its matching animation clip; exit stops any loop it started.*
...
```mermaid
flowchart TD
  A["JumpState entered"] --> B["Play jump_launch once"]
  B --> C["finished flag set true"]
  C --> D["Play jump_airborne loop"]
  D --> E{"Apex reached"}
  E -- not yet --> D
  E -- yes --> F["FSM transition to FallState"]
```
*One FSM state can host two animation clips in sequence; only the apex check triggers an actual FSM transition.*
