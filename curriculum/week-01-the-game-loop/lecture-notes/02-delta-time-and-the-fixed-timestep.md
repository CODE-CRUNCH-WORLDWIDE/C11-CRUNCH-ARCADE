...
```mermaid
flowchart TD
  A["Add dt to accumulator"] --> B{"Accumulator at least one sim step"}
  B -->|Yes| C["Advance physics by one sim step"]
  C --> D["Subtract sim step from accumulator"]
  D --> B
  B -->|No| E["Render at current state"]
  E --> F["Get next dt from clock tick"]
  F --> A
```
*The accumulator loop lets simulation always advance in fixed steps while render runs at whatever rate it can.*
...
```mermaid
flowchart TD
  A["Choose input style"] --> B{"Fires once per press"}
  B -->|Yes| C["Use KEYDOWN event"]
  B -->|No| D["Use get_pressed every frame"]
  C --> E["Jump fire open menu"]
  D --> F["Movement charging weapon"]
```
*Whether the action should fire once or repeat while held decides which input style to use.*
