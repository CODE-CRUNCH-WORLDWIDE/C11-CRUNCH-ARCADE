...
flowchart LR
  ...
  C --> A
```
*Every frame runs input then update then render, then loops back to input.*
...
```mermaid
flowchart LR
  A["Draw into back buffer"] --> B["Call flip"]
  B --> C["Back and front buffers swap"]
  C --> D["Monitor reads new front buffer"]
  D --> A
```
*Double buffering: you draw into the back buffer while the monitor reads the front, then flip swaps them.*
