...
**8.** A learner writes the shader below and assigns it to two Sprite2D nodes that share the same `ShaderMaterial` resource. When they call `node_a.material.set_shader_parameter("flash", 1.0)`, both sprites flash. Why? How do you fix it in one line?
...
**17.** A learner is configuring a `GPUParticles2D` and sets `mat.gravity = Vector2(0, 400)`. Godot raises an error. Why?
...
(d) The learner must enable the `gravity_enabled` flag first.
