...
Every exercise this week ships with a working, commented file. The "solution" is therefore not a fresh implementation — it is a *walk-through* of the file in the order a learner should read it, plus the answers to the "Things to try" prompts at the bottom of each exercise.
...
4. **Defensive checks in `_ready()`**. Warn if the input action is missing; warn if the material is not a `ShaderMaterial`. These warnings save learners an hour of "why doesn't it work" debugging on first run.
