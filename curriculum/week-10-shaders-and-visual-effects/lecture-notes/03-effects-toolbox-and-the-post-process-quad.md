...
```mermaid
flowchart LR
  A["Player takes damage"] --> B["add_shake 6.0"]
  A --> C["flash uniform to 1.0 over 120 ms"]
  A --> D["hit_stop 80 ms"]
  B --> E["Camera shakes and decays"]
  C --> F["Sprite flashes white then fades"]
  D --> G["Game briefly freezes"]
```
*One damage event fans out into the three polish-tripod effects at once.*
...
A note on the gravity field: even in `GPUParticles2D`, the gravity field is a `Vector3`. The z-component is ignored for 2D, but you must provide it. The trap catches every learner once.
