...
```mermaid
flowchart LR
  A["Pass 1 renders sprite to a texture"] --> B["Texture now holds neighbour data"]
  B --> C["Pass 2 samples the texture at several UVs"]
  C --> D["Fragment shader combines the samples"]
```
*Two-pass rendering is how a fragment learns about pixels it cannot read directly.*
