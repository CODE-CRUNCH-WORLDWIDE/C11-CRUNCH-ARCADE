...
```mermaid
flowchart TD
  A["gdshader file - the program"] --> C["ShaderMaterial"]
  B["Uniform values - the parameters"] --> C
  C --> D["Sprite2D node"]
```
*A ShaderMaterial pairs one shader program with a bag of uniform values; the node just references it.*
...
```mermaid
sequenceDiagram
  participant Game as on_hit script
  participant Tween as Tween
  participant Mat as ShaderMaterial
  participant GPU as Fragment shader
  Game->>Mat: set flash to 1.0
  Game->>Tween: create_tween and tween_method
  loop every frame for 120 ms
    Tween->>Mat: set flash to interpolated value
    Mat->>GPU: uniform flash read each fragment
  end
  Tween->>Mat: set flash to 0.0
```
*A hit-flash is a tween driving a uniform every frame until it reaches zero.*
