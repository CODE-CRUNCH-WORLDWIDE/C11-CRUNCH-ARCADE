...
```mermaid
flowchart TD
  A["New audio clip"] --> B{"Decoded size over 1 MB"}
  B -- "No" --> C["Use Sound"]
  B -- "Yes" --> D["Stream with music"]
  C --> E["Fire and forget SFX"]
  D --> F["One background track"]
```

*Choosing between the RAM-resident Sound path and the streaming music path.*
...
sequenceDiagram
  ...
  participant Ch as Channel 0
  participant EvLoop as Event loop
  Game->>Ch: set_endevent DIALOGUE_END_EVENT
  Game->>Ch: play dialogue_clip
  Ch-->>EvLoop: post DIALOGUE_END_EVENT
  EvLoop->>Game: restore_music_after_duck
```

*How a channel's end-of-clip event reaches the game loop without polling.*
