...
```mermaid
flowchart TD
  A["New audio clip"] --> B{"Does it loop"}
  B -- "Yes" --> C["Use OGG - never MP3"]
  B -- "No" --> D{"Longer than 5 seconds"}
  D -- "Yes" --> E["Use OGG for music and dialogue"]
  D -- "No" --> F["Use WAV for short SFX"]
```

*Picking a file format from clip length and whether the clip loops.*
...
```mermaid
flowchart TD
  A["Click at the seam"] --> B{"Short pop under 5 ms"}
  B -- "Yes" --> C["Not a zero-crossing - recut with Z snap"]
  B -- "No" --> D{"Audible silence at seam"}
  D -- "Yes" --> E["Trailing silence - trim and re-export"]
  D -- "No" --> F{"Click repeats every loop, same spot"}
  F -- "Yes" --> G["DC offset - Normalize and remove DC offset"]
  F -- "No" --> H{"File is MP3"}
  H -- "Yes" --> I["Priming delay - convert to OGG"]
  H -- "No" --> J["Cross fade the seam"]
```

*The same diagnostic tree as the ASCII version above, as a flowchart.*
