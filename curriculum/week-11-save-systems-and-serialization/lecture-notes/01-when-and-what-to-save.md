...
```mermaid
flowchart LR
  A["Player data"] --> B["Meta bucket settings and achievements"]
  A --> C["Run bucket character inventory world flags"]
  B --> D["meta json write on settings close"]
  C --> E["slot files write on save autosave checkpoint"]
```

*Meta and run state split into separate files so a corrupted save cannot destroy settings, and vice versa.*
...
```mermaid
flowchart LR
  A["Manual save"] --> B["Chosen slot file"]
  A --> C["Free save file"]
  D["Autosave"] --> C
  C --> E["Continue button"]
  B --> F["Slot list"]
...
*Manual saves write to both the chosen slot and the free save file; autosaves write only to the free save, which backs the Continue button.*
