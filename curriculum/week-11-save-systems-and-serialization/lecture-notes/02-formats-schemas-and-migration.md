...
```mermaid
flowchart TD
  A["Bytes read from disk"] --> B{"How are they deserialized"}
  B -->|"Construct whatever type the bytes name"| C["pickle BinaryFormatter unserialize"]
  C --> D["Attacker controlled code can run"]
  B -->|"Parse against a fixed schema"| E["Pydantic Protobuf typed JSON"]
  E --> F["Unknown or malformed data is rejected"]
```

*The unsafe path lets the file choose what code runs; the safe path only ever produces values the schema already named.*
...
```mermaid
flowchart LR
  A["v1 save"] --> B["migrate v1 to v2"]
  B --> C["v2 save"]
  C --> D["migrate v2 to v3"]
  D --> E["v3 save"]
  E --> F["migrate v3 to v4"]
  F --> G["v4 save current"]
```

*Each version step gets its own migration function; loading an old save chains through them one at a time.*
