...
```mermaid
flowchart LR
  A["Write payload to save tmp"] --> B["fsync save tmp"]
  B --> C["rename save tmp to save latest"]
  C --> D["Production file updated atomically"]
```

*A crash at any point before the rename leaves the previous save.latest untouched.*
...
```mermaid
flowchart TD
  A["Start load"] --> B{"Production file readable and valid"}
  B -->|"Yes"| C["Load production"]
  B -->|"No"| D{"Previous file readable and valid"}
  D -->|"Yes"| E["Load previous and warn"]
  D -->|"No"| F["No good save found"]
```

*The loader falls back from the production save to the previous backup before giving up.*
